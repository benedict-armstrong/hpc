...
if __name__ == "__main__":
    ...
    plt.title(f"Weak benchmark ({data['runs']} runs, first removed)")
    ...
    fig, axs = plt.subplots(1, len(data["sizes"]), figsize=(15, 10))
    ...
    for i, size in enumerate(data["sizes"]):
        ...
        average_times = [sum(t) / len(t) for t in times_for_size]

        t_1 = average_times[0]
        x_values = data["threads"]
        y_values = [t_1 / t for t in average_times]

        confidence_95 = 1.96 * math.sqrt(
            sum([(t_1 - t)**2 for t in average_times]) / (len(average_times) - 1))

        axs[i].errorbar(x_values, y_values,
                        yerr=confidence_95,
                        capsize=3,
                        label=f"Base size {size}; {data['runs']} runs, first removed)")

        axs[i].set_xlabel("Number of OpenMP threads")
        axs[i].set_ylabel("Efficiency")
        axs[i].grid()
        # set title for each subplot
        axs[i].set_title(
            f"Base size {size}; {data['runs']} runs, first removed")
